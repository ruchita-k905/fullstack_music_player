...
const getSongs = async (req, res) => {
  try {
    const response = await axios.get(
      `https://api.jamendo.com/v3.0/tracks/?client_id=34ca7511&format=jsonpretty&limit=15`,
    );
...
const getPlaylistByTag = async (req, res) => {
  try {
    ...
    console.error('getPlaylistTag error', error?.response?.data ?? error);
